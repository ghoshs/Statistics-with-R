...
ctable<-table(dative$LengthOfTheme)
# The table shows the frequency of the values of variable LenghtOfTheme
...
library(ggplot2)
hist(ctable)
ggplot(dative, aes(LengthOfTheme))+geom_histogram()
# The data is positively skewed
ggplot(dative, aes(x="",y=LengthOfTheme))+geom_boxplot()
# There is one outlier as is visible in the box plot with LengthOfTheme > 45.
...
# Distribution describes the number of times each value of a variable occurs. 
# It describes a population in terms of absolute frequency or density. 
# A sampling distribution describes the number of times each value of a variable 
# occurs in a sample. It is a subset of the population.
...
mean5 <- rep(0, 1000)
# mean5 <- numeric(length(1000)) # preallocation
# 2. mean5<- c() 
# 3. mean5<- NULL
for(i in 1:1000){
  mean5[i] <- mean(sample(dative$LengthOfTheme, 5)) # also for 2. 
  # 3. mean5 <- c(mean5, mean(sample(dative$LengthOfTheme, 5)))
}
...
mean50 <- rep(0, 1000)
for(i in 1:1000){
  mean50[i] <- mean(sample(dative$LengthOfTheme, 50))
}

## j) Explain in your own words what 'means5' and 'means50' now contain. 
##    How do they differ?
# mean5 (mean50) is a sample distribution of means where each value is mean of sample 
# drawn from the LengthOfTheme distribution with sample size is 5 (50).
# The distribution mean50 is more closely located around the actual mean of LengthOfTheme
# than the mean5 distribution due to the difference in sample size
...
# mean5 has a positive skew

## l) What causes this skew? In other words, why does means5 have bigger 
##    maximum numbers than means50?
# The actual data is positively skewed, and since we have not had enough draws 
# in the sample distribuition of mean5, the distribution of sample of means 
# also has a positive skew. If the samples
# for 5 draws has an outlier, its effect on the mean will be more than the 
# effect of the outliers on a sample with 50 draws. As the sample size is 
# increased to 50, the distribution resembles a normal curve with more samples closer 
# to the mean of LengthOfTime.
max(mean5)
max(mean50)
# Larger samples -> standard error less -> reliable estimates
###############
...
# We use the confidence parameter to show a range within which the population parameter
# that we are trying to estimate may lie. Thus if we compute a 95% confidence interval,
# we imply that if the experiment is replicated 100 times, 95 out of 100 times, the 
# population parameter would lie within the confidence interval.
...
##    What does this mean?
# mean(dative$LengthOfTheme) = 4.271529
# mean(mean50) = 4.2899
# mean(mean5) = 4.3208
# confidence interval is [4.121943, 4.421115]. The sample means (mean5, mean50)
# fall into the this interval. For small sample size, usually the mean does not fall 
...
sd(subset(dative$LengthOfTheme, dative$AnimacyOfTheme== 'animate'))
sd(subset(dative$LengthOfTheme, dative$AnimacyOfTheme== 'inanimate'))
# sd(inanimate) > sd(animate) 
length(subset(dative$LengthOfTheme, dative$AnimacyOfTheme== 'animate'))
length(subset(dative$LengthOfTheme, dative$AnimacyOfTheme== 'inanimate'))
# But, #samples in inanimate > animate -> CI of inanimate is very narrow
# remember standard error = sd(population)/sqrt(#samples)
...
bargraph.CI(AnimacyOfTheme, LengthOfTheme, data=dative, ci.fun=ciMean)
# by default the ci for bargraph is one standard error away from the mean which is less
# than 95% confidence level. The confidence interval returned by ciMean is
# by default for 95% confidence level.
...
ggplot(data=ratings.2, aes(x=condition, y=frequency, fill=condition)) +
  geom_bar(stat="identity", show.legend=F) +
  guides(fill=FALSE) # removes legend
...
ggplot(ratings.3, aes(x=occurrence, y=frequency, group=condition, 
                      color=condition, shape=factor(condition)))+
...
# People talk more frequently about exotic animals then common animals.
# In case of plants, people talk more frequently about common plants than exotic plants.